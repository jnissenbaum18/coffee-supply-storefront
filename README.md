...
1. Confirm `public/mockServiceWorker.js` exists.
...
3. Open browser DevTools → **Application** → **Service Workers**.
   ...
   Confirm a worker for `mockServiceWorker.js` is active.

4. If an old worker appears stuck, click **Unregister**, clear site data, then hard-refresh.
...
5. Stop and restart Vite after generating/replacing the worker:
...
6. Confirm the mock worker starts in `src/main.tsx` before `createRoot(...).render(...)`.

7. Confirm the handler path matches the request path:
